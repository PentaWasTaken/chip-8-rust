...
impl Display for Chip8Error {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        match *self {
            ...
            Chip8Error::UnsupportedInstr(instr, pc) => write!(
                f,
                "Unsupported instruction: 0x{:x} on position {}",
                instr, pc
            ),
            Chip8Error::ReturnOnEmptyStack(pc) => {
                write!(f, "Return on empty stack on position {}", pc)
            }
        }
    }
}
